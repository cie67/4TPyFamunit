...
from _4TPyModel.code.buildergrid import buildergrid, consedocodpostal, cestados
...
@app.route('/convertirenventa', methods=['POST', 'GET'])
def convertirenventa():
...
#hace busquedas en tablas con el operador like
    ...
    for cadafile in onlyfiles:
        if contador==0:
            if ".png" in cadafile.lower()  or ".jpg" in cadafile.lower()  or ".bmp" in cadafile.lower()  or ".gif" in cadafile.lower() or ".jpeg" in cadafile.lower()    :
                losfiles +='<img style="height:220px;"    src="/static/uploads/' + tabla + '/' + idobjeto + '/' + cadafile + '" />\r\n'
                contador +=1
...
def upload_file():
    ...
    return redirect('formrender?campo1=' + campo1 + '&valor1=' + valor1 + '&vistadetalle=' + vistadetalle + '&form=tables/' + tabla + '_form&idobjeto=' + objeto + '&viewmode=files')
...
def excelexport():
    ...
    if buscar=='' or buscar=='null' or buscar==None:
        ...
        data=buscadorxlsx("select *,concat('<a href=""javascript:modoedicion(',ID" + tablename.replace('tbl','') + ",');"">Editar</a>') as EDITAR from " + tablename.lower() + " where " + quierolaclasusulawhere(tablename,buscar))
...
def populator():
    ...
    if buscar=='' or buscar=='null' or buscar==None:
        ...
        return buscador("select *,concat('<a href=""javascript:modoedicion(',ID" + tablename.replace('tbl','') + ",');"">Editar</a>') as EDITAR from " + tablename.lower() + " where " + quierolaclasusulawhere(tablename,buscar),usuario,idusername,tipouser,esadmin)
...
def validaracceso():
    ...
    idusername = request.args.get('idusername')
    tablename = request.args.get('tablename')
    #print('AQUI VA: ' + idusername + '   ' + tablename )
...
#JICE - Agregados 04052020

@app.route('/estados', methods=['GET', 'POST'])
def estados():
    #idusername = request.args.get('idusername')
    #tablename = request.args.get('tablename')
    print('CONSULTA DE ESTADOS')
    # print(checarpermisos(tablename,idusername) + 'HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH')
    #respuesta = dict()
    #respuesta = {'': cestados()}
    #respuestaconsedocodpostal()
    #print(respuesta.values())

    return jsonify(cestados())




@app.route('/consultaedo', methods=['GET', 'POST'])
def consultaedo():
    #idusername = request.args.get('idusername')
    #tablename = request.args.get('tablename')
    print('CONSULTA DE COMPLETA')
    # print(checarpermisos(tablename,idusername) + 'HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH')
    respuesta = dict()
    respuesta = {'Direcciones': consedocodpostal()}
    #respuestaconsedocodpostal()
    #print(respuesta.values())

    return respuesta
    #json.load(consedocodpostal())
        #str(checarpermisos(tablename, idusername))
