...
class builderadaptation:
    def builderadaptation(tablename):
            ...
            #if os.path.exists(os.path.join(app.root_path,'templates', 'tables', tablename + '_population.4tpy')):
            f = open(  os.path.join(app.root_path,'templates', 'tables', 'tblaltanotify_' + tablename +  '_population.4tpy')   , "w",encoding='utf8')
            ...
            #if os.path.exists(os.path.join(app.root_path,'templates', 'tables', tablename + '_population.4tpy')):
            f = open(  os.path.join(app.root_path,'templates', 'tables', 'tblestatusnotify_' + tablename +  '_population.4tpy')   , "w",encoding='utf8')
            ...
            sentencia= "update tblconfigcampos_" + tablename + " set editable=0,orden='-10000', mostrarcampo=1 where campo='ID" + tablename.replace('tbl','').upper() + "' or campo='FECHAALTA' or campo='CREADOR' or campo='BLOQUEADO' or campo='ELIMINADO'   ;"
            resultado=dbexecutor.executor(sentencia)
            ...
            sentencia= "CREATE PROCEDURE " + tablename + "_PROC_CAMPODISPLAY (IN _id INT(11))\r\n BEGIN  \r\n  UPDATE " + tablename + " SET CAMPODISPLAY=CONCAT(' '," + 'ID' + tablename.replace('tbl','') + ",'-','-',ifnull(" + campox[1] + ",''),'-',ifnull(" + campox[2] + ",''),'-',ifnull(" + campox[3] + ",'')) WHERE " +  'ID' + tablename.replace('tbl','') + "=_id;    \r\n  \r\n END" 
            #print(sentencia)
            resultado=dbexecutor.executor(sentencia)
            ...
            sentencia= "CREATE PROCEDURE " + tablename + "_PROC_AFTER_UPDATE (IN _id INT(11))\r BEGIN \r update  " + tablename + " set guid=uuid() where (guid is null  or guid='') and " + 'ID' + tablename.replace('tbl','') + "=_id;\r END" 
            resultado=dbexecutor.executor(sentencia)
            ...
            if os.path.exists(  os.path.join(app.root_path,'templates', 'tables', tablename +  '_body_form_inclusion.html') ):
                ...
                f = open( os.path.join(app.root_path,'templates', 'tables', tablename +  '_body_form_inclusion.html'), "w",encoding='utf8')
                f.write('')
                f.close() 

            #archivo en header form inclussion    
            if os.path.exists( os.path.join(app.root_path,'templates', 'tables', tablename +  '_header_form_inclusion.html')):
                pass
            else:
                f = open( os.path.join(app.root_path,'templates', 'tables', tablename +  '_header_form_inclusion.html'), "w",encoding='utf8')
                f.write('')
                f.close()
            ...
            if os.path.exists( os.path.join(app.root_path,'templates', 'tables', tablename +  '_form_buttons.html')):
                ...
                f = open( os.path.join(app.root_path,'templates', 'tables', tablename +  '_form_buttons.html'), "w",encoding='utf8')
            ...
            if os.path.exists( os.path.join(app.root_path,'templates', 'tables', tablename +  '_grid_buttons.html')):
                ...
                f = open( os.path.join(app.root_path,'templates', 'tables', tablename +  '_grid_buttons.html'), "w",encoding='utf8')
            ...
            if os.path.exists( os.path.join(app.root_path,'templates', 'tables', tablename +  '_header_inclusion.4tpy')):
                ...
                f = open( os.path.join(app.root_path,'templates', 'tables', tablename +  '_header_inclusion.4tpy'), "w",encoding='utf8')
                f.write('')
                f.close()
            ...
            #else:
            f = open( os.path.join(app.root_path,'templates', 'tables', tablename +  '_population.4tpy'), "w",encoding='utf8')
            f.write("select " + quierotodosloscampos(tablename) + ", concat('<a style=\"color:blue;\" href=\"javascript:modoedicion('," + campox[0] + ",');\"" + ">Editar</a>') as EDITAR from " + tablename + "  order by " + campox[0] + " desc")
                #f.write('select * from ' + tablename + ' order by ID' + tablename.replace('tbl','') + ' desc '  )
            f.close()


            f = open( os.path.join(app.root_path,'templates', 'tables', 'tblestatus_' + tablename +  '_population.4tpy'), "w",encoding='utf8')
            f.write('select * from tblestatus_' + tablename )
                #f.write('select * from ' + tablename + ' order by ID' + tablename.replace('tbl','') + ' desc '  )
            f.close()

            f = open( os.path.join(app.root_path,'templates', 'tables', 'tblconfigcampos_' + tablename +  '_population.4tpy'), "w",encoding='utf8')
            ...
            if os.path.exists( os.path.join(app.root_path,'templates', 'tables', tablename +  '_grid_top_buttons.4tpy')):
                ...
                f = open( os.path.join(app.root_path,'templates', 'tables', tablename +  '_grid_top_buttons.4tpy'), "w",encoding='utf8')
            ...
            if os.path.exists( os.path.join(app.root_path,'templates', 'tables', tablename +  '_css.css')):
                ...
                f.write('<style>	h5{	color: #982d2d;    font-family: pompadour; font-size: 19px;    font-variant: small-caps;}	td{		color:#0014ff	;    font-family: pompadour;}</style>')
                f.close()
