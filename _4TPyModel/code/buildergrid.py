...
def cestados():
    ...
    mycursor = mydb.cursor()
    #laorden = "SELECT concat('Colonia:', ASENTAMIENTO, ',Municipio:', MUNICIPIO, ',Estado:', ESTADO, ',Codigo Postal:', CODIGOPOSTAL) as CODPOSTAL from tblcodpostal order by estado asc"
    laorden = "SELECT ESTADO from view_estados order by estado asc"
    # print(laorden)
    mycursor.execute(laorden)
    myresult = mycursor.fetchall()
...
def consedocodpostal():
    ...
    mycursor = mydb.cursor()
    #laorden = "SELECT concat('Colonia:', ASENTAMIENTO, ',Municipio:', MUNICIPIO, ',Estado:', ESTADO, ',Codigo Postal:', CODIGOPOSTAL) as CODPOSTAL from tblcodpostal order by estado asc"
    laorden = "SELECT ASENTAMIENTO, MUNICIPIO, ESTADO,CODIGOPOSTAL from tblcodpostal order by estado asc"
    # print(laorden)
    mycursor.execute(laorden)
    myresult = mycursor.fetchall()
    ...
    resultado = ''
    for result in myresult:
        content = {'Colonia': result[0], 'Municipio': result[1], 'Estado': result[2], 'Codigo postal': result[3]}
        payload.append(content)
        content = {}
        resultado = str(result[0])
