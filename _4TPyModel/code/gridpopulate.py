...
def ordencampos(tablename, tipoorden):
    ...
    mycursor = mydb.cursor()
    mycursor.execute(
        "SELECT GRUPO, ORDEN,CAMPO, CAPTION, MOSTRARCAMPO, EDITABLE, ANCHO, ALTO, HELPTEXT,  OBLIGATORIO,BUILDTYPE, MOSTRARGRID FROM tblconfigcampos_" + tablename.lower() + " WHERE MOSTRARCAMPO=1 ORDER BY grupo ASC, orden ASC")
    myresult = mycursor.fetchall()
